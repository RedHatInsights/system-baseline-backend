...
if [ -z "$ACG_CONFIG" ]; then
  ...
  prometheus_multiproc_dir=$TEMPDIR gunicorn wsgi --worker-class uvicorn.workers.UvicornWorker -w $NUM_WORKERS --threads $THREADS -b 0.0.0.0:$PORT --log-level=$LOG_LEVEL --access-logfile=- --config ./gunicorn.conf.py
  ...
  if [[ "$?" != "0" ]]; then exit 1; fi
  exec gunicorn wsgi --worker-class uvicorn.workers.UvicornWorker --reload --bind=0.0.0.0:"$PORT" --bind=0.0.0.0:"$METRICS_PORT" --log-level="$LOG_LEVEL" --access-logfile=- --limit-request-field_size="$GUNICORN_REQUEST_FIELD_LIMIT" --config "$APP_CONFIG"
fi
rm -rf $TEMPDIR
